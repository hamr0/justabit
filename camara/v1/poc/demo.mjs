// PoC M6 — the one-command demo. Run: node camara/v1/poc/demo.mjs [--backend mock|orange]
//
...
export async function main(argv = process.argv.slice(2), { createBackendImpl = createBackend } = {}) {
  ...
  if (args.error) {
    console.error(`${args.error}\nusage: node camara/v1/poc/demo.mjs [--backend mock|orange]`);
    return 2;
  }
  let backend;
  try {
...
// Only when RUN, never when imported — camara/v1/poc/m6-check.mjs imports the seams above
