...
| D3 | **Agent/delegation proposal** — agent/delegation side only, targets the IETF OAuth WG. Now versioned per project like CAMARA: v1 is the frozen record of `-00` as posted 2026-08-31; v2 is the frozen record of `-01`, SUBMITTED and posted 2026-09-02 (expires 2027-03-06): all four scope items and Appendix A direction 4 drafted; two review rounds run and their findings fixed; the PoC catch-up done; author-tools and idnits both ran clean on the submitted bytes; v3 is the `-02` working copy, not yet submitted. | `ietf/v1/docs/ietf-agent-delegation.md` (v1); `ietf/v2/docs/draft-hamr-oauth-agent-delegation-01.xml` (v2, posted); `ietf/v3/docs/draft-hamr-oauth-agent-delegation-02.xml` (v3, working) | IETF (OAuth WG); supersedes `docs/archive/aaif-agent-auth.md`, retained only as a dated, superseded record of the earlier AAIF-targeted text |
...
|---|---|---|---|
| 2026-09-03 | asor -01 posted, confirmed live against the fetched bytes (not the email): `min` constraint type present in §4.2, acknowledgment present in Appendix A with the `tenureMin` correction taken, `tenure_years` correctly stays in asor's own example, asor -01 cites the HAMR draft. The -01 XML's "not present ... as posted" sentence (lines 1484-1489) is NOT YET FALSE because the cited reference (lines 1739-1747) is still pinned to asor-00; it becomes false the moment -02 bumps that reference to -01, so -02 must do both together — bump the reference AND rewrite the sentence — as a paired task, or the document contradicts itself. Posted -01 is never edited for this | active | [findings.md](../logs/findings.md#2026-09-03--asor-01-posted-acknowledgment-confirmed-live-against-the-bytes-hamr-02-reference-bump-and-rewrite-pair-identified) |
